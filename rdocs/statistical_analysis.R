...
cat("### 5.4 Type III F-tests (Satterthwaite)\n\n")
cat("```\n")
print(anova(model_signed))
cat("```\n\n")

# R-squared
r2_signed <- r.squaredGLMM(model_signed)
cat("### 5.5 Variance Explained (R-squared)\n\n")
cat("| Type | R² |\n")
cat("|------|----|\n")
...
cat("### 5.6 Intraclass Correlation (ICC)\n\n")
...
cat("### 5.7 Fixed Effects with 95% CI\n\n")
...
cat("### 5.8 Random Effects Variance\n\n")
cat("```\n")
...
# Parsimonious model: spatial factors non-significant (LRT p=.83) so excluded
...
# Use REML = TRUE for better F-test inference (Satterthwaite df)
...
cat("### 9.2 Type III F-tests (Satterthwaite)\n\n")
cat("```\n")
print(anova(model_spatial))
cat("```\n\n")

cat("### 9.3 Post-Hoc: Azimuth Sectors\n\n")
cat("```\n")
emm_az <- emmeans(model_spatial, ~ azimuthSector)
print(pairs(emm_az))
cat("```\n\n")

cat("### 9.4 Post-Hoc: Elevation Categories\n\n")
cat("```\n")
